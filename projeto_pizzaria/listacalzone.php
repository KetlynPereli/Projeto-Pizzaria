...
<body>
    ...
    <div class="fundo_calzone">
        <div class="form">
            ...
            <form action="excluircalzone.php" method="post">
                <center>
                    <h1>Calzones Cadastrados</h1>
                    <table id="tabela" border="1">
                        <thead>
                            <th>Código do Calzone</th>
                            <th>Nome do Calzone</th>
                            <th>Valor</th>
                        ...
                        $dados = mysqli_query($conexao, "SELECT * FROM `calzone`");

                        while ($item = mysqli_fetch_assoc($dados)) {
                        ?>
                            <tr>
                                <td><?= @$item['id_calzone'] ?></td>
                                <td><?= @$item['calzone'] ?></td>
                                <td><?= @$item['valor_calzone'] ?></td>
                                <td><a href="editarcalzone.php?id_calzone=<?php echo @$item['id_calzone']; ?>"><i class="fa fa-pencil"></a></td>
                                <td onClick="verifica('<?= @$item["id_calzone"]; ?>')"><a href="#"><i class="fa fa-trash"></a></td>
                            </tr>
                    ...
                    <script>
                        function verifica(recid) {
                            if (confirm("Tem certeza que deseja Excluir permanentemente este Calzone?")) {
                                window.location = "excluircalzone.php?id_calzone=" + recid
                            }
                        }
                    </script>
