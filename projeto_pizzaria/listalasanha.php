    ...
    <div class="fundo_lasanha">
        <div class="form">
            <form action="excluirnhoque.php" method="post">
                <center>
                    <h1>Lasanhas Cadastradas</h1>
                    <table id="tabela" border="1">
                        <thead>
                            <th>Código da Lasanha</th>
                            <th>Nome da Lasanha</th>
                            <th>Valor</th>
                        ...
                        $dados = mysqli_query($conexao, "SELECT * FROM `lasanha`");

                        while ($item = mysqli_fetch_assoc($dados)) {
                        ?>
                            <tr>
                                <td><?= @$item['id_lasanha'] ?></td>
                                <td><?= @$item['lasanha'] ?></td>
                                <td><?= @$item['valor_lasanha'] ?></td>
                                <td><a href="editarlasanha'.php?id_lasanha<?php echo @$item['id_lasanha']; ?>"><i class="fa fa-pencil"></a></td>
                                <td onClick="verifica('<?= @$item["id_lasanha"]; ?>')"><a href="#"><i class="fa fa-trash"></a></td>
                            </tr>
                    ...
                    <script>
                        function verifica(recid) {
                            if (confirm("Tem certeza que deseja Excluir permanentemente esta Lasanha'")) {
                                window.location = "excluirlasanha.php?id_lasanha'" + recid
                            }
                        }
                    </script>
