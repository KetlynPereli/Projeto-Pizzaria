...
<body>
    ...
    <div class="fundo_sobremesa">
        <div class="form">
            <form action="excluirsobremesa.php" method="post">
                <center>
                    <h1>Sobremesas Cadastradas</h1>
                    <table id="tabela" border="1">
                        <thead>
                            <th>Código da Sobremesa</th>
                        ...
                        $dados = mysqli_query($conexao, "SELECT * FROM `sobremesa`");

                        while ($item = mysqli_fetch_assoc($dados)) {
                        ?>
                            <tr>
                                <td><?= @$item['id_sobremesa'] ?></td>
                                <td><?= @$item['sobremesa'] ?></td>
                                <td><?= @$item['valor_sobremesa'] ?></td>
                                <td><a href="editarsobremesa.php?id_sobremesa=<?php echo @$item['id_sobremesa']; ?>"><i class="fa fa-pencil"></a></td>
                                <td onClick="verifica('<?= @$item["id_sobremesa"]; ?>')"><a href="#"><i class="fa fa-trash"></a></td>
                            </tr>
                    ...
                    <script>
                        function verifica(recid) {
                            if (confirm("Tem certeza que deseja Excluir permanentemente este Sanduiche?")) {
                                window.location = "excluirsobremesa.php?id_sobremesa=" + recid
                            }
                        }
                    </script>
