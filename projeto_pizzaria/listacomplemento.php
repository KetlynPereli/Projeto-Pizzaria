...
<head>
    ...
    <title>Lista de Complementos</title>
</head>
...
<body>
    ...
    <div class="fundo_complemento">
        <div class="form">

            <form action="excluircomplemento.php" method="post">
                <center>
                    <h1>Complementos Cadastrados</h1>
                    <table id="tabela" border="1">
                        <thead>
                            <th>Código do Complemento</th>
                            <th>Nome do Complemento</th>
                            <th>Valor</th>
                        ...
                        $dados = mysqli_query($conexao, "SELECT * FROM `complementos`");

                        while ($item = mysqli_fetch_assoc($dados)) {
                        ?>
                            <tr>
                                <td><?= @$item['id_complementos'] ?></td>
                                <td><?= @$item['complemento'] ?></td>
                                <td><?= @$item['valor_complemento'] ?></td>
                                <td><a href="editarcomplemento.php?id_complementos=<?php echo @$item['id_complementos']; ?>"><i class="fa fa-pencil"></a></td>
                                <td onClick="verifica('<?= @$item["id_complementos"]; ?>')"><a href="#"><i class="fa fa-trash"></a></td>
                            </tr>
                    ...
                    <script>
                        function verifica(recid) {
                            if (confirm("Tem certeza que deseja Excluir permanentemente este Complemento?")) {
                                window.location = "excluircomplemento.php?id_complementos=" + recid
                            }
                        }
                    </script>
