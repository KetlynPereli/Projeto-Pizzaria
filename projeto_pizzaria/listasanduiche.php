...
<head>
    ...
    <title>Lista de Sanduiches</title>
</head>
...
<body>
    ...
    <div class="fundo_sanduiche">
        <div class="form">
            <form action="excluirsanduiche.php" method="post">
                <center>
                    <h1>Sanduiches Cadastrados</h1>
                    <table id="tabela" border="1">
                        <thead>
                            <th>Código do Sanduiche</th>
                            <th>Nome do Sanduiche</th>
                            <th>Valor</th>
                        ...
                        $dados = mysqli_query($conexao, "SELECT * FROM `sanduiche`");

                        while ($item = mysqli_fetch_assoc($dados)) {
                        ?>
                            <tr>
                                <td><?= @$item['id_sanduiche'] ?></td>
                                <td><?= @$item['sanduiche'] ?></td>
                                <td><?= @$item['valor_sanduiche'] ?></td>
                                <td><a href="editarsanduiche.php?id_sanduiche=<?php echo @$item['id_sanduiche']; ?>"><i class="fa fa-pencil"></a></td>
                                <td onClick="verifica('<?= @$item["id_sanduiche"]; ?>')"><a href="#"><i class="fa fa-trash"></a></td>
                            </tr>
                    ...
                    <script>
                        function verifica(recid) {
                            if (confirm("Tem certeza que deseja Excluir permanentemente este Sanduiche?")) {
                                window.location = "excluirsanduiche.php?id_sanduiche=" + recid
                            }
                        }
                    </script>
