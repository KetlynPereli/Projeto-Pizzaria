...
<body>
    ...
    <div class="fundo_bebida">
        <div class="form">
            <form action="excluirbebidas.php" method="post">
                <center>
                    ...
                    <table id="tabela" border="1">
                        ...
                        $dados = mysqli_query($conexao, "SELECT * FROM `bebidas`");

                        while ($item = mysqli_fetch_assoc($dados)) {
                        ?>
                            <tr>
                                <td><?= @$item['id_bebidas'] ?></td>
                                <td><?= @$item['bebida'] ?></td>
                                <td><?= @$item['valor_bebida'] ?></td>
                                <td><a href="editarbebidas.php?id_bebidas=<?php echo @$item['id_bebidas']; ?>"><i class="fa fa-pencil"></a></td>
                                <td onClick="verifica('<?= @$item["id_bebidas"]; ?>')"><a href="#"><i class="fa fa-trash"></a></td>
                            </tr>
                    ...
                    <script>
                        function verifica(recid) {
                            if (confirm("Tem certeza que deseja Excluir permanentemente esta Bebida?")) {
                                window.location = "excluirbebidas.php?id_bebidas=" + recid
                            }
                        }
                    </script>
                </center>
