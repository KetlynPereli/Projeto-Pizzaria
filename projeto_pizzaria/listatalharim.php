...
<head>
    ...
    <title>Lista de Talharim ou Rondeli</title>
</head>
...
<body>
    ...
    <div class="fundo_talharim">
        <div class="form">
            <form action="excluirsobremesa.php" method="post">
                <center>
                    <h1>Talharins ou Rondelis Cadastrados</h1>
                    <table id="tabela" border="1">
                        <thead>
                            <th>Código do Talharim ou Rondeli</th>
                            <th>Nome do Talharim ou Rondeli</th>
                            <th>Valor</th>
                        ...
                        $dados = mysqli_query($conexao, "SELECT * FROM `talharim_e_rondeli`");

                        while ($item = mysqli_fetch_assoc($dados)) {
                        ?>
                            <tr>
                                <td><?= @$item['id_talharim'] ?></td>
                                <td><?= @$item['talharim_rondeli'] ?></td>
                                <td><?= @$item['valor_talharim'] ?></td>
                                <td><a href="editartalharim.php?id_talharim=<?php echo @$item['id_talharim']; ?>"><i class="fa fa-pencil"></a></td>
                                <td onClick="verifica('<?= @$item["id_talharim"]; ?>')"><a href="#"><i class="fa fa-trash"></a></td>
                            </tr>
                    ...
                    <script>
                        function verifica(recid) {
                            if (confirm("Tem certeza que deseja Excluir permanentemente este Talharim ou Rondeli?")) {
                                window.location = "excluirtalharim.php?id_talharim=" + recid
                            }
                        }
                    </script>
